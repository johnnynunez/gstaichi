...
@banner("Build GsTaichi Android C-API Shared Library")
def build_android(python: Command, pip: Command) -> None:
    """
    Build the GsTaichi Android C-API shared library
    """
    ...
    os.environ["GSTAICHI_FORCE_PLAT_NAME"] = "android-arm64"  # affects setup.py
    pip.install("-r", "requirements_dev.txt")
    python("setup.py", "clean")
    python("setup.py", "build_ext")
    for p in Path(os.getcwd()).glob("**/libgstaichi_c_api.so"):
