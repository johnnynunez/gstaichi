...
def cache_files_size(path: str) -> int:
    files = listdir(path)
    result = 0
    for file in files:
        ...
        elif is_offline_cache_file(file):
...
def cache_files_cnt(path: str | None = None) -> int:
    if path is None:
        return cache_files_cnt(tmp_offline_cache_file_path())
    count = 0
    try:
