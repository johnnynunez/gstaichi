...
GsTaichi Core exposes all necessary interfaces for offloading the AOT modules to GsTaichi. The following is a list of features that are available regardless of your backend. The corresponding APIs are still under development and subject to change.

## Availability

GsTaichi C-API intends to support the following backends:
...
For convenience, in the following text and other C-API documents, the term *host* refers to the user of the C-API; the term *device* refers to the logical (conceptual) compute device, to which GsTaichi's runtime offloads its compute tasks. A *device* may not be a physical discrete processor other than the CPU and the *host* may *not* be able to access the memory allocated on the *device*.
...
The following section provides a brief introduction to the GsTaichi C-API.

### Create and destroy a Runtime Instance

You *must* create a runtime instance before working with GsTaichi, and *only* one runtime per thread. Currently, we do not officially claim that multiple runtime instances can coexist in a process, but please feel free to [file an issue with us](https://github.com/taichi-dev/gstaichi/issues) if you run into any problem with runtime instance coexistence.

```cpp
// Create a GsTaichi Runtime on Vulkan device at index 0.
...
### Load and destroy a GsTaichi AOT module

You can load a GsTaichi AOT module from the filesystem.
...
A sentinal invalid handle that will never be produced from a valid call to GsTaichi C-API.

`handle.runtime`

GsTaichi runtime represents an instance of a logical backend and its internal dynamic state. The user is responsible to synchronize any use of `handle.runtime`. The user *must not* manipulate multiple `handle.runtime`s in the same thread.

`handle.aot_module`

An ahead-of-time (AOT) compiled GsTaichi module, which contains a collection of kernels and compute graphs.
...
A GsTaichi kernel that can be launched on the offload target for execution.

`handle.compute_graph`

A collection of GsTaichi kernels (a compute graph) to launch on the offload target in a predefined order.

`enumeration.error`

Errors reported by the GsTaichi C-API.

- `enumeration.error.success`: The GsTaichi C-API invocation finished gracefully.
- `enumeration.error.not_supported`: The invoked API, or the combination of parameters is not supported by the GsTaichi C-API.
...
- `enumeration.error.invalid_state`: The GsTaichi C-API enters an unrecoverable invalid state. Related GsTaichi objects are potentially corrupted. The users *should* release the contaminated resources for stability. Please feel free to file an issue if you encountered this error in a normal routine.
...
Usages of a memory allocation. GsTaichi requires kernel argument memories to be allocated with `bit_field.memory_usage.storage`.
...
Usages of an image allocation. GsTaichi requires kernel argument images to be allocated with `bit_field.image_usage.storage` and `bit_field.image_usage.sampled`.
...
Get the current gstaichi version. It has the same value as `TI_C_API_VERSION` as defined in `gstaichi_core.h`.
...
Gets the last error raised by GsTaichi C-API invocations. Returns the semantical error code.
...
Sets the provided error as the last error raised by GsTaichi C-API invocations. It can be useful in extended validation procedures in GsTaichi C-API wrappers and helper libraries.
...
Creates a GsTaichi Runtime with the specified `enumeration.arch`.

- `function.create_runtime.arch`: Arch of GsTaichi Runtime.
- `function.create_runtime.device_index`: The index of device in `function.create_runtime.arch` to create GsTaichi Runtime on.

`function.destroy_runtime`

Destroys a GsTaichi Runtime.
...
Tracks the device image with the provided image layout. Because GsTaichi tracks image layouts internally, it is *only* useful to inform GsTaichi that the image is transitioned to a new layout by external procedures.

`function.transition_image`

Transitions the image to the provided image layout. Because GsTaichi tracks image layouts internally, it is *only* useful to enforce an image layout for external procedures to use.

`function.launch_kernel`

Launches a GsTaichi kernel with the provided arguments. The arguments *must* have the same count and types in the same order as in the source code.

`function.launch_compute_graph`

Launches a GsTaichi compute graph with provided named arguments. The named arguments *must* have the same count, names, and types as in the source code.
...
Retrieves a pre-compiled GsTaichi kernel from the AOT module.
