// # Core Functionality
//
// GsTaichi Core exposes all necessary interfaces for offloading the AOT modules
// to GsTaichi. The following is a list of features that are available regardless
...
// logical (conceptual) compute device, to which GsTaichi's runtime offloads its
...
//
// You *must* create a runtime instance before working with GsTaichi, and *only*
...
// issue with us](https://github.com/taichi-dev/gstaichi/issues) if you run into
...
//
// GsTaichi runtime represents an instance of a logical backend and its internal
...
//
// An ahead-of-time (AOT) compiled GsTaichi module, which contains a collection of
...
//
// A collection of GsTaichi kernels (a compute graph) to launch on the offload
...
typedef enum TiError {
  // The GsTaichi C-API invocation finished gracefully.
  TI_ERROR_SUCCESS = 0,
  ...
  // The GsTaichi C-API enters an unrecoverable invalid state. Related GsTaichi
...
//
// Usages of a memory allocation. GsTaichi requires kernel argument memories to be
...
//
// Usages of an image allocation. GsTaichi requires kernel argument images to be
...
//
// Get the current gstaichi version. It has the same value as `TI_C_API_VERSION`
// as defined in `gstaichi_core.h`.
...
//
// Gets the last error raised by GsTaichi C-API invocations. Returns the
...
//
// Sets the provided error as the last error raised by GsTaichi C-API invocations.
// It can be useful in extended validation procedures in GsTaichi C-API wrappers
...
TI_DLL_EXPORT TiRuntime TI_API_CALL ti_create_runtime(
    // Arch of GsTaichi Runtime.
    TiArch arch,
    // The index of device in `function.create_runtime.arch` to create GsTaichi
    // Runtime on.
...
//
// Tracks the device image with the provided image layout. Because GsTaichi tracks
// image layouts internally, it is *only* useful to inform GsTaichi that the image
...
//
// Transitions the image to the provided image layout. Because GsTaichi tracks
...
//
// Launches a GsTaichi kernel with the provided arguments. The arguments *must*
...
//
// Launches a GsTaichi compute graph with provided named arguments. The named
