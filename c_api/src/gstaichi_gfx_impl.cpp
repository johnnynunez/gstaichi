...
GfxRuntime::GfxRuntime(gstaichi::Arch arch) : Runtime(arch) {
}

Error GfxRuntime::create_aot_module(const gstaichi::io::VirtualDir *dir,
                                    TiAotModule &out) {
  gstaichi::lang::gfx::AotModuleParams params{};
  params.dir = dir;
  params.runtime = &get_gfx_runtime();
  std::unique_ptr<gstaichi::lang::aot::Module> aot_module =
      gstaichi::lang::aot::Module::load(arch, params);
  if (aot_module->is_corrupted()) {
    return Error(TI_ERROR_CORRUPTED_DATA, "aot_module");
  }

  const gstaichi::lang::DeviceCapabilityConfig &current_devcaps =
      params.runtime->get_ti_device()->get_caps();
  const gstaichi::lang::DeviceCapabilityConfig &required_devcaps =
  ...
  for (const auto &pair : required_devcaps.devcaps) {
    ...
    if (current_version != required_version) {
      return Error(TI_ERROR_INCOMPATIBLE_MODULE,
                   gstaichi::lang::to_string(pair.first).c_str());
    }
  }
...
void GfxRuntime::buffer_copy(const gstaichi::lang::DevicePtr &dst,
                             const gstaichi::lang::DevicePtr &src,
                             size_t size) {
  get_gfx_runtime().buffer_copy(dst, src, size);
}
void GfxRuntime::copy_image(const gstaichi::lang::DeviceAllocation &dst,
...
void GfxRuntime::track_image(const gstaichi::lang::DeviceAllocation &image,
                             gstaichi::lang::ImageLayout layout) {
  get_gfx_runtime().track_image(image, layout);
}
void GfxRuntime::untrack_image(const gstaichi::lang::DeviceAllocation &image) {
  get_gfx_runtime().untrack_image(image);
}
void GfxRuntime::transition_image(const gstaichi::lang::DeviceAllocation &image,
                                  gstaichi::lang::ImageLayout layout) {
