...
#include "gstaichi_opengl_impl.h"
#include "gstaichi_vulkan_impl.h"
#include "gstaichi_llvm_impl.h"
...
#include "gstaichi/common/utils.h"

bool is_vulkan_available() {
#ifdef TI_WITH_VULKAN
  return gstaichi::lang::vulkan::is_vulkan_api_available();
#else
  return false;
#endif
}
...
bool is_opengl_available() {
  ...
  return gstaichi::lang::opengl::is_opengl_api_available();
#else
  return false;
#endif
}
...
#ifdef TI_WITH_CUDA
  return gstaichi::is_cuda_api_available();
#else
  return false;
#endif
}
...
bool is_metal_available() {
  ...
  return gstaichi::lang::metal::is_metal_api_available();
#else
  return false;
#endif  // __APPLE__
}
...
Runtime::Runtime(gstaichi::Arch arch) : arch(arch) {
}
Runtime::~Runtime() {
}

VulkanRuntime *Runtime::as_vk() {
  TI_ASSERT(arch == gstaichi::Arch::vulkan);
#ifdef TI_WITH_VULKAN
...
capi::MetalRuntime *Runtime::as_mtl() {
  TI_ASSERT(arch == gstaichi::Arch::metal);
#ifdef TI_WITH_METAL
...
TiMemory Runtime::allocate_memory(
  ...
  gstaichi::lang::RhiResult res = this->get().allocate_memory(params, &devalloc);
  if (res != gstaichi::lang::RhiResult::success) {
    throw std::bad_alloc();
  }
...
gstaichi::lang::aot::Kernel *AotModule::get_kernel(const std::string &name) {
  return aot_module_->get_kernel(name);
}
gstaichi::lang::aot::CompiledGraph *AotModule::get_cgraph(
...
gstaichi::lang::aot::Module &AotModule::get() {
  return *aot_module_;
}
...
TiRuntime ti_create_runtime(TiArch arch, uint32_t device_index) {
  ...
  TI_CAPI_TRY_CATCH_BEGIN();
  // TI_INFO("GsTaichi Runtime C-API version is: {}", TI_C_API_VERSION);
  switch (arch) {
#ifdef TI_WITH_VULKAN
    case TI_ARCH_VULKAN: {
    ...
    case TI_ARCH_X64: {
      ...
      out = (TiRuntime)(static_cast<Runtime *>(
          new capi::LlvmRuntime(gstaichi::Arch::x64)));
      break;
    }
    case TI_ARCH_ARM64: {
      TI_CAPI_NOT_SUPPORTED_IF_RV(device_index != 0);
      out = (TiRuntime)(static_cast<Runtime *>(
          new capi::LlvmRuntime(gstaichi::Arch::arm64)));
      break;
    }
    case TI_ARCH_CUDA: {
      TI_CAPI_NOT_SUPPORTED_IF_RV(device_index != 0);
      out = (TiRuntime)(static_cast<Runtime *>(
          new capi::LlvmRuntime(gstaichi::Arch::cuda)));
      break;
    }
#endif  // TI_WITH_LLVM
#ifdef TI_WITH_METAL
...
void ti_set_runtime_capabilities_ext(
  ...
  gstaichi::lang::DeviceCapabilityConfig devcaps;
  for (uint32_t i = 0; i < capability_count; ++i) {
    const auto &cap_level_info = capabilities[i];
    devcaps.set((gstaichi::lang::DeviceCapability)cap_level_info.capability,
...
void ti_get_runtime_capabilities(TiRuntime runtime,
  ...
  Runtime *runtime2 = (Runtime *)runtime;
  const gstaichi::lang::DeviceCapabilityConfig &devcaps =
...
TiMemory ti_allocate_memory(TiRuntime runtime,
  ...
  if (create_info->usage & TI_MEMORY_USAGE_STORAGE_BIT) {
    usage = usage | gstaichi::lang::AllocUsage::Storage;
  }
  if (create_info->usage & TI_MEMORY_USAGE_UNIFORM_BIT) {
    usage = usage | gstaichi::lang::AllocUsage::Uniform;
  }
  if (create_info->usage & TI_MEMORY_USAGE_VERTEX_BIT) {
    usage = usage | gstaichi::lang::AllocUsage::Vertex;
  }
  if (create_info->usage & TI_MEMORY_USAGE_INDEX_BIT) {
    usage = usage | gstaichi::lang::AllocUsage::Index;
  }
...
TiImage ti_allocate_image(TiRuntime runtime,
  ...
  gstaichi::lang::ImageAllocUsage usage{};
  if (allocate_info->usage & TI_IMAGE_USAGE_STORAGE_BIT) {
    usage = usage | gstaichi::lang::ImageAllocUsage::Storage;
  }
  if (allocate_info->usage & TI_IMAGE_USAGE_SAMPLED_BIT) {
    usage = usage | gstaichi::lang::ImageAllocUsage::Sampled;
  }
  if (allocate_info->usage & TI_IMAGE_USAGE_ATTACHMENT_BIT) {
    usage = usage | gstaichi::lang::ImageAllocUsage::Attachment;
  }

  switch ((gstaichi::lang::ImageDimension)allocate_info->dimension) {
#define PER_IMAGE_DIMENSION(x) case gstaichi::lang::ImageDimension::x:
...
#undef PER_IMAGE_DIMENSION
  ...
  switch ((gstaichi::lang::BufferFormat)allocate_info->format) {
#define PER_BUFFER_FORMAT(x) case gstaichi::lang::BufferFormat::x:
#include "gstaichi/inc/rhi_constants.inc.h"
#undef PER_BUFFER_FORMAT
  ...
  gstaichi::lang::ImageParams params{};
  params.x = allocate_info->extent.width;
  params.y = allocate_info->extent.height;
  params.z = allocate_info->extent.depth;
  params.dimension = (gstaichi::lang::ImageDimension)allocate_info->dimension;
  params.format = (gstaichi::lang::BufferFormat)allocate_info->format;
...
void ti_copy_image_device_to_device(TiRuntime runtime,
  ...
  gstaichi::lang::ImageCopyParams params{};
...
void ti_track_image_ext(TiRuntime runtime,
  ...
  auto layout2 = (gstaichi::lang::ImageLayout)layout;

  switch ((gstaichi::lang::ImageLayout)layout) {
#define PER_IMAGE_LAYOUT(x) case gstaichi::lang::ImageLayout::x:
#include "gstaichi/inc/rhi_constants.inc.h"
...
TiAotModule ti_create_aot_module(TiRuntime runtime,
  ...
  auto dir = gstaichi::io::VirtualDir::from_zip(tcm, size);
...
TiKernel ti_get_aot_module_kernel(TiAotModule aot_module, const char *name) {
  ...
  gstaichi::lang::aot::Kernel *kernel =
...
TiComputeGraph ti_get_aot_module_compute_graph(TiAotModule aot_module,
  ...
  gstaichi::lang::aot::CompiledGraph *cgraph =
...
void ti_launch_kernel(TiRuntime runtime,
  ...
  if (arg_count > gstaichi_max_num_args_total) {
    ti_set_last_error(TI_ERROR_ARGUMENT_OUT_OF_RANGE, "arg_count");
    return;
  }

  auto ti_kernel = (gstaichi::lang::aot::Kernel *)kernel;

  Runtime &runtime2 = *((Runtime *)runtime);
  gstaichi::lang::LaunchContextBuilder builder(ti_kernel);
  std::vector<std::unique_ptr<gstaichi::lang::DeviceAllocation>> devallocs;
  ...
  for (uint32_t i = 0; i < arg_count; ++i) {
    ...
    switch (arg.type) {
      ...
      case TI_ARGUMENT_TYPE_NDARRAY: {
        ...
        std::unique_ptr<gstaichi::lang::DeviceAllocation> devalloc =
            std::make_unique<gstaichi::lang::DeviceAllocation>(
...
void ti_launch_compute_graph(TiRuntime runtime,
  ...
  Runtime &runtime2 = *((Runtime *)runtime);
  std::unordered_map<std::string, gstaichi::lang::aot::IValue> arg_map{};
  std::vector<gstaichi::lang::Ndarray> ndarrays;
  ndarrays.reserve(arg_count);
  std::vector<gstaichi::lang::Texture> textures;
  textures.reserve(arg_count);
  std::vector<gstaichi::lang::Matrix> matrices;
  ...
  for (uint32_t i = 0; i < arg_count; ++i) {
    ...
    switch (arg.argument.type) {
        ...
        switch (arg.argument.value.scalar.type) {
          case TI_DATA_TYPE_I16: {
            ...
            arg_map.emplace(std::make_pair(
                arg.name, gstaichi::lang::aot::IValue::create<int16_t>(arg_val)));
            break;
          }
          case TI_DATA_TYPE_U16: {
            ...
            arg_map.emplace(std::make_pair(
                arg.name,
                gstaichi::lang::aot::IValue::create<uint16_t>(arg_val)));
            break;
          }
          case TI_DATA_TYPE_F16: {
            float arg_val;
            std::memcpy(&arg_val, &arg.argument.value.scalar.value.x32,
                        sizeof(arg_val));
            arg_map.emplace(std::make_pair(
                arg.name, gstaichi::lang::aot::IValue::create<float>(arg_val)));
            break;
          }
          default: {
            ti_set_last_error(
      ...
      case TI_ARGUMENT_TYPE_I32: {
        arg_map.emplace(
            std::make_pair(arg.name, gstaichi::lang::aot::IValue::create<int32_t>(
      ...
      case TI_ARGUMENT_TYPE_F32: {
        arg_map.emplace(std::make_pair(
            arg.name,
            gstaichi::lang::aot::IValue::create<float>(arg.argument.value.f32)));
        break;
      }
      case TI_ARGUMENT_TYPE_NDARRAY: {
        ...
        const gstaichi::lang::DataType *prim_ty;
        switch (ndarray.elem_type) {
          case TI_DATA_TYPE_F16:
            prim_ty = &gstaichi::lang::PrimitiveType::f16;
            break;
          case TI_DATA_TYPE_F32:
            prim_ty = &gstaichi::lang::PrimitiveType::f32;
            break;
          case TI_DATA_TYPE_F64:
            prim_ty = &gstaichi::lang::PrimitiveType::f64;
            break;
          case TI_DATA_TYPE_I8:
            prim_ty = &gstaichi::lang::PrimitiveType::i8;
            break;
          case TI_DATA_TYPE_I16:
            prim_ty = &gstaichi::lang::PrimitiveType::i16;
            break;
          case TI_DATA_TYPE_I32:
            prim_ty = &gstaichi::lang::PrimitiveType::i32;
            break;
          case TI_DATA_TYPE_I64:
            prim_ty = &gstaichi::lang::PrimitiveType::i64;
            break;
          case TI_DATA_TYPE_U8:
            prim_ty = &gstaichi::lang::PrimitiveType::u8;
            break;
          case TI_DATA_TYPE_U16:
            prim_ty = &gstaichi::lang::PrimitiveType::u16;
            break;
          case TI_DATA_TYPE_U32:
            prim_ty = &gstaichi::lang::PrimitiveType::u32;
            break;
          case TI_DATA_TYPE_U64:
            prim_ty = &gstaichi::lang::PrimitiveType::u64;
            break;
          case TI_DATA_TYPE_GEN:
            prim_ty = &gstaichi::lang::PrimitiveType::gen;
            break;
          default: {
            ti_set_last_error(TI_ERROR_ARGUMENT_OUT_OF_RANGE,
        ...
        gstaichi::lang::DataType dtype = *prim_ty;
        if (elem_shape.size() > 0) {
          dtype = gstaichi::lang::TypeFactory::get_instance().get_tensor_type(
              elem_shape, dtype);
        }
        ndarrays.emplace_back(gstaichi::lang::Ndarray(devalloc, dtype, shape));
        arg_map.emplace(std::make_pair(
            arg.name, gstaichi::lang::aot::IValue::create(ndarrays.back())));
        break;
      }
      case TI_ARGUMENT_TYPE_TEXTURE: {
        ...
        gstaichi::lang::BufferFormat format =
            (gstaichi::lang::BufferFormat)arg.argument.value.texture.format;
        ...
        textures.emplace_back(
            gstaichi::lang::Texture(devalloc, format, width, height, depth));
        arg_map.emplace(std::make_pair(
            arg.name, gstaichi::lang::aot::IValue::create(textures.back())));
        break;
      }
      case TI_ARGUMENT_TYPE_TENSOR: {
        ...
        gstaichi::lang::DataType dtype = *prim_ty;
        matrices.emplace_back(gstaichi::lang::Matrix(length, dtype, data));
        arg_map.emplace(std::make_pair(
            arg.name, gstaichi::lang::aot::IValue::create(matrices.back())));
        break;
      }
      default: {
        ti_set_last_error(
  ...
  ((gstaichi::lang::aot::CompiledGraph *)compute_graph)->run(arg_map);
  TI_CAPI_TRY_CATCH_END();
}
