...
LlvmRuntime::LlvmRuntime(gstaichi::Arch arch) : Runtime(arch) {
  cfg_ = std::make_unique<gstaichi::lang::CompileConfig>();
  cfg_->arch = arch;

  executor_ =
      std::make_unique<gstaichi::lang::LlvmRuntimeExecutor>(*cfg_.get(), nullptr);
...
gstaichi::lang::Device &LlvmRuntime::get() {
  gstaichi::lang::Device *device = executor_->get_compute_device();
  return *device;
}

TiMemory LlvmRuntime::allocate_memory(
    const gstaichi::lang::Device::AllocParams &params) {
  gstaichi::lang::LLVMRuntime *llvm_runtime = executor_->get_llvm_runtime();
  gstaichi::lang::LlvmDevice *llvm_device = executor_->llvm_device();
...
void LlvmRuntime::free_memory(TiMemory devmem) {
  const gstaichi::lang::CompileConfig &config = executor_->get_config();
  // For memory allocated through Device::allocate_memory_runtime(),
  // the corresponding Device::free_memory() interface has not been
  // implemented yet...
  if (gstaichi::arch_is_cpu(config.arch)) {
    TI_CAPI_NOT_SUPPORTED_IF(gstaichi::arch_is_cpu(config.arch));
  }
...
TiAotModule LlvmRuntime::load_aot_module(const char *module_path) {
  ...
  std::unique_ptr<gstaichi::lang::aot::Module> aot_module{nullptr};

  if (gstaichi::arch_is_cpu(config.arch)) {
    gstaichi::lang::cpu::KernelLauncher::Config cfg;
    cfg.executor = executor_.get();
    gstaichi::lang::LLVM::AotModuleParams aot_params;
    aot_params.executor_ = executor_.get();
    aot_params.kernel_launcher =
        std::make_unique<gstaichi::lang::cpu::KernelLauncher>(std::move(cfg));
    aot_params.module_path = module_path;
    aot_module = gstaichi::lang::LLVM::make_aot_module(std::move(aot_params));
  } else {
#ifdef TI_WITH_CUDA
    TI_ASSERT(config.arch == gstaichi::Arch::cuda);
    gstaichi::lang::cuda::KernelLauncher::Config cfg;
    cfg.executor = executor_.get();
    gstaichi::lang::LLVM::AotModuleParams aot_params;
    aot_params.executor_ = executor_.get();
    aot_params.kernel_launcher =
        std::make_unique<gstaichi::lang::cuda::KernelLauncher>(std::move(cfg));
    aot_params.module_path = module_path;
    aot_module = gstaichi::lang::LLVM::make_aot_module(std::move(aot_params));
#else
    TI_NOT_IMPLEMENTED;
  ...
  */
  auto *llvm_aot_module =
      dynamic_cast<gstaichi::lang::LLVM::LlvmAotModule *>(aot_module.get());
  ...
  for (size_t i = 0; i < llvm_aot_module->get_num_snode_trees(); i++) {
    ...
    gstaichi::lang::LLVM::allocate_aot_snode_tree_type(
...
void LlvmRuntime::buffer_copy(const gstaichi::lang::DevicePtr &dst,
...
void ti_export_cpu_memory(TiRuntime runtime,
  ...
  if (((Runtime *)runtime)->arch != gstaichi::Arch::x64 &&
      ((Runtime *)runtime)->arch != gstaichi::Arch::arm64) {
  ...
  auto &cpu_device = static_cast<gstaichi::lang::cpu::CpuDevice &>(device);
...
void ti_export_cuda_memory(TiRuntime runtime,
  ...
  TI_CAPI_ARGUMENT_NULL(memory);
  TI_CAPI_ARGUMENT_NULL(interop_info);
  if (((Runtime *)runtime)->arch != gstaichi::Arch::cuda) {
  ...
  auto &device = llvm_runtime->get();
  auto &cuda_device = static_cast<gstaichi::lang::cuda::CudaDevice &>(device);
...
#ifdef TI_WITH_CUDA
  gstaichi::lang::CUDAContext::get_instance().set_stream(stream);
...
#ifdef TI_WITH_CUDA
  *stream = gstaichi::lang::CUDAContext::get_instance().get_stream();
#else
  TI_NOT_IMPLEMENTED;
