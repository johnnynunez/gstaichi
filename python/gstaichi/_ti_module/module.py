...
import gstaichi
from gstaichi._ti_module.cppgen import generate_header
from gstaichi.aot._export import _aot_kernels
...
from gstaichi.types.primitive_types import integer_type_ids, real_type_ids
from gstaichi.types.texture_type import RWTextureType, TextureType


def module_cppgen(parser: argparse.ArgumentParser):
    """Generate C++ headers for GsTaichi modules."""
...
def module_cppgen_impl(a):
    ...
    print(f"Generating C++ header for GsTaichi module: {Path(module_path).absolute()}")
...
def module_build(parser: argparse.ArgumentParser):
    """Build GsTaichi modules from python scripts."""
    parser.add_argument("SOURCE", help="Path to the GsTaichi program source (Python script).")
...
def module_build_impl(a):
    ...
    print(f"Building GsTaichi module: {source_path}")
    print()
        ...
        if record.template_types:
            ...
            for k, v in record.template_types.items():
                ...
                if isinstance(v, int) or id(v) in integer_type_ids:
                    ...
                    value = gstaichi.ndarray(v.dtype, (1,) * v.ndim)
                elif isinstance(v, TextureType):
                    value = gstaichi.Texture(gstaichi.Format.rgba8, (4,) * v.num_dimensions)
                elif isinstance(v, RWTextureType):
                    value = gstaichi.Texture(v.fmt, (4,) * v.num_dimensions)
                else:
...
def _main(arguments: List[str]):
    """GsTaichi module tools."""
    parser = argparse.ArgumentParser(prog="ti module", description=_main.__doc__)
    subparsers = parser.add_subparsers(title="GsTaichi module manager commands", required=True)
