...
from gstaichi.aot.utils import produce_injected_args
from gstaichi.lang import impl, kernel_impl
...
from gstaichi.types import enums
from gstaichi.types.texture_type import FORMAT2TY_CH, TY_CH2FORMAT
...
class Graph:
    ...
    def run(self, args):
        ...
        # GsTaichi Matrix types are flattened into (int, float) arrays.
        # TODO diminish the flatten behavior when Matrix becomes a GsTaichi native type.
        flattened = {}
    ...
    for k, v in kwargs.items():
        ...
        if k == "tag":
            if not isinstance(v, ArgKind):
                raise GsTaichiRuntimeError(f"tag must be a ArgKind variant, but found {type(v)}.")
        if k == "name":
            if not isinstance(v, str):
                raise GsTaichiRuntimeError(f"name must be a string, but found {type(v)}.")
...
def _make_arg_scalar(kwargs: Dict[str, Any]):
    ...
    dtype = kwargs["dtype"]
    if isinstance(dtype, MatrixType):
        raise GsTaichiRuntimeError(f"Tag ArgKind.SCALAR must specify a scalar type, but found {type(dtype)}.")
...
def _make_arg_ndarray(kwargs: Dict[str, Any]):
    ...
    if isinstance(dtype, MatrixType):
        raise GsTaichiRuntimeError(f"Tag ArgKind.NDARRAY must specify a scalar type, but found {dtype}.")
...
def _make_arg_matrix(kwargs: Dict[str, Any]):
    ...
    if not isinstance(dtype, MatrixType):
        raise GsTaichiRuntimeError(f"Tag ArgKind.MATRIX must specify matrix type, but got {dtype}.")
...
def _make_arg_rwtexture(kwargs: Dict[str, Any]):
    ...
    if fmt == enums.Format.unknown:
        raise GsTaichiRuntimeError(f"Tag ArgKind.RWTEXTURE must specify a valid color format, but found {fmt}.")
...
def _kwarg_rewriter(args, kwargs):
    for i, arg in enumerate(args):
        ...
        if i in rewrite_map:
            ...
            raise GsTaichiRuntimeError(f"Unexpected {i}th positional argument")
