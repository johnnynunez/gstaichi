...
# Instant-NGP renderer implemented using GsTaichi
# Author : Linyou (linyoutian.loyot@gmail.com)
# Original code at https://github.com/Linyou/gstaichi-ngp-renderer
...
from gstaichi.math import uvec3, vec2, vec3
...
try:
    ...
    raise ImportError("The GsTaichi NGP renderer requires cv2, wget and scipy to be installed.")
...
PRETRAINED_MODEL_URL = "https://github.com/Linyou/gstaichi-ngp-renderer/releases/download/v0.1-models/{}.npy"
...
class NGP_fw:
    ...
    @staticmethod
    def gstaichi_init(kernel_profiler):
        ti.init(
            arch=arch,
            offline_cache=True,
    ...
    def raymarching_test_kernel(self, N_samples: int):
        ...
        for n in ti.ndrange(self.counter[None]):
            ...
            while (0 <= t) & (t < t2) & (s < N_samples):
                ...
                # occ = density_grid_gstaichi[idx] > 5.912066756501768
    ...
    def write_image(self):
        ...
        plt.imsave("gstaichi_ngp.png", (rgb_np * 255).astype(np.uint8))
        plt.imsave("gstaichi_ngp_depth.png", depth2img(depth_np))
    ...
    def render_gui(self):
        ...
        W, H = self.res
        window = ti.ui.Window("GsTaichi NGP", (W, H))
...
def main(args):
    NGP_fw.gstaichi_init(args.print_profile)
    res = args.res
    scale = 0.5
    ngp = NGP_fw(
        scale=scale,
    ...
    if args.print_profile:
        NGP_fw.gstaichi_print_profiler()
