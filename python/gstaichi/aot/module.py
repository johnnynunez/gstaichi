...
import gstaichi
from gstaichi.aot.utils import produce_injected_args, produce_injected_args_from_template
from gstaichi.lang import impl, kernel_impl
from gstaichi.lang.field import ScalarField
from gstaichi.lang.matrix import MatrixField
    ...
    def keygen(v, key_p, fields):
        ...
        for ky, val in fields:
            if val is v:
                ...
                return key_p
        raise RuntimeError(
            "Arg type must be of type int/float/boolean" f" or gstaichi field. Type {str(type(v))}" " is not supported"
        )
...
class Module:
    ...
    def __init__(self, arch=None, caps=None):
        ...
        Args:
          arch: Target backend architecture. Default to the one initialized in :func:`~gstaichi.lang.init` if not specified.
    ...
    def add_field(self, name, field):
        ...
        Args:
          name: name of gstaichi field
          field: gstaichi field
    ...
    def add_kernel(self, kernel_fn, template_args=None, name=None):
        ...
        Args:
          kernel_fn (Function): the function decorated by gstaichi `kernel`.
    ...
    def save(self, filepath):
        ...
        with open(f"{filepath}/__version__", "w") as f:
            f.write(".".join(str(x) for x in gstaichi.__version__))
