...
from gstaichi.lang.ast.function_def_transformer import FunctionDefTransformer
...
class BoundFunc:
    def __init__(self, fn: Callable, instance: Any, gstaichi_callable: "GsTaichiCallable"):
        self.fn = fn
    ...
    def __setattr__(self, k: str, v: Any) -> None:
        if k in ("fn", "instance", "gstaichi_callable"):
...
class GsTaichiCallable:
    def __init__(self, fn: Callable, wrapper: Callable) -> None:
        ...
        self._is_gstaichi_function = False
...
def func(fn: Callable, is_real_function=False) -> GsTaichiCallable:
...
class Kernel:
    ...
    #   const DeviceCapabilityConfig &caps);
        ...
        gstaichi_kernel = prog.create_kernel(gstaichi_ast_generator, kernel_name, self.autodiff_mode)
        ...
        def set_arg_ext_array(indices: tuple[int, ...], v: Any, needed: ndarray_type.NdarrayType) -> None:
            ...
            if isinstance(v, np.ndarray):
                ...
                if isinstance(v, torch.Tensor):
                    ...
                    assert prog is not None
                    gstaichi_arch = prog.config().arch
        ...
        def recursive_set_args(needed: Type, provided: Type, v: Any, indices: tuple[int, ...]) -> int:
            ...
            if isinstance(needed, ArgPackType):
                if not isinstance(v, ArgPack):
                    raise GsTaichiRuntimeTypeError.get(indices, str(needed), str(provided))
                idx_new = 0
            ...
            if id(needed) in primitive_types.real_type_ids:
                if not isinstance(v, (float, int, np.floating, np.integer)):
                    raise GsTaichiRuntimeTypeError.get(indices, needed.to_string(), provided)
                if in_argpack:
                    return 1
            ...
            if dataclasses.is_dataclass(needed):
                ...
                return idx
            if isinstance(needed, ndarray_type.NdarrayType) and isinstance(v, gstaichi.lang._ndarray.Ndarray):
                if in_argpack:
                    set_later_list.append((set_arg_ndarray, (v,)))
                    return 0
                set_arg_ndarray(indices, v)
                return 1
            if isinstance(needed, texture_type.TextureType) and isinstance(v, gstaichi.lang._texture.Texture):
                if in_argpack:
                    set_later_list.append((set_arg_texture, (v,)))
                    return 0
                set_arg_texture(indices, v)
                return 1
            if isinstance(needed, texture_type.RWTextureType) and isinstance(v, gstaichi.lang._texture.Texture):
                if in_argpack:
            ...
            if isinstance(needed, StructType):
                ...
                if not isinstance(v, needed):  # type: ignore Might be invalid? Maybe should rewrite as: not needed.__instancecheck__(v) ?
                    raise GsTaichiRuntimeTypeError(f"Argument {provided} cannot be converted into required type {needed}")
                needed.set_kernel_struct_args(v, launch_ctx, indices)
                return 1
...
class _BoundedDifferentiableMethod:
    def __init__(self, kernel_owner: Any, wrapped_kernel_func: GsTaichiCallable | BoundFunc):
    ...
    def _getattr(self, item):
        ...
        if hasattr(x, "_is_wrapped_kernel"):
            if inspect.ismethod(x):
                ...
                wrapped = x
            assert isinstance(wrapped, (BoundFunc, GsTaichiCallable))
