...
from gstaichi.lang.kernel_impl import BoundFunc, GsTaichiCallable
...
def create_field_member(dtype, name, needs_grad, needs_dual):
    ...
    prog = get_runtime().prog
    if prog is None:
        raise GsTaichiRuntimeError("Cannont create field, maybe you forgot to call `ti.init()` first?")
...
def ndarray(dtype, shape, needs_grad=False):
    ...
    prog = get_runtime().prog
    if prog is None:
        raise GsTaichiRuntimeError("Cannont create ndarray, maybe you forgot to call `ti.init()` first?")
...
def static(x, *xs) -> Any:
    ...
    if isinstance(x, (FunctionType, MethodType, BoundFunc, GsTaichiCallable)):
