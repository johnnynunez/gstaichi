...
class Canvas:
    ...
    please call the `get_canvas()` method of :class:`~gstaichi.ui.Window`.
    """
    ...
    def set_image(self, img):
        ...
        Args:
            img (numpy.ndarray, :class:`~gstaichi.MatrixField`, :class:`~gstaichi.Field`, :class:`~gstaichi.Texture`): \
                the image to be shown.
        """
        is_texture = isinstance(img, Texture)
        prog_is_vk = impl.pygstaichi.prog.config().arch == _ti_core.Arch.vulkan
    ...
    def triangles(self, vertices, color=(0.5, 0.5, 0.5), indices=None, per_vertex_color=None):
        ...
        Args:
            vertices: a gstaichi 2D Vector field, where each element indicate \
                the 3D location of a vertex.
            indices: a gstaichi int field of shape (3 * #triangles), which \
            ...
            per_vertex_color (Tuple[float]): a gstaichi 3D vector field, \
    ...
    def circles(self, centers, radius, color=(0.5, 0.5, 0.5), per_vertex_color=None, per_vertex_radius=None):
        ...
        Args:
            centers: a gstaichi 2D Vector field, where each element indicate the \
                2D location of the center of a vertex.
            radius: radius of the circles in pixels.
            color: a global color for the triangles as 3 floats representing \
                RGB values. If `per_vertex_color` is provided, this is ignored.
            per_vertex_color: a gstaichi 3D vector field, where \
                each element indicates the RGB color of a circle.
            per_vertex_radius: a gstaichi float field, where \
    ...
    def vector_field(self, vector_field, arrow_spacing=5, scale=0.1, width=0.002, color=(0, 0, 0)):
        ...
        try:
            ...
            raise RuntimeError("Can't import gstaichi and/or numpy.")
    ...
    def scene(self, scene):
        ...
        Args:
            scene (:class:`~gstaichi.ui.Scene`): an instance of :class:`~gstaichi.ui.Scene`.
        """
