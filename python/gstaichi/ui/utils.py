...
from gstaichi._lib.utils import try_get_wheel_tag
...
def get_field_info(field):
    ...
    if isinstance(field, np.ndarray):
        ...
        info.dtype = to_gstaichi_type(field.dtype)
        info.num_elements = np.prod(field.shape)
        info.shape = field.shape
        return info
    if isinstance(field, Ndarray):
        info.field_source = _ti_core.FieldSource.GsTaichiNDarray
...
def check_ggui_availability():
    ...
    try:
        ...
        import gstaichi  # pylint: disable=import-outside-toplevel

        wheel_tag = try_get_wheel_tag(gstaichi)
        if platform.system() == "Linux" and wheel_tag and "manylinux2014" in wheel_tag:
            raise GGUINotAvailableException(
                "GGUI is not available since you have installed a restricted version of gstaichi. "
