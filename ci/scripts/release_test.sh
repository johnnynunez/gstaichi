...
function gstaichi::test::ggui {
    ...
    # divider
    gstaichi::utils::line
    gstaichi::utils::logger::info "Running GGUI examples"

    # clone the repo
    gstaichi::utils::git_clone "${ORG}" "${REPO}"
    cd "${REPO}/python/gstaichi/examples/ggui_examples"
...
function gstaichi::test::main {
    ...
    gstaichi::test::generate_videos "${WORKDIR}"
}
