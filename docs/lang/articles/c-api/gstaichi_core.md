...
GsTaichi runtime represents an instance of a logical backend and its internal dynamic state. The user is responsible to synchronize any use of [`TiRuntime`](#handle-tiruntime). The user *must not* manipulate multiple [`TiRuntime`](#handle-tiruntime)s in the same thread.
...
- `TI_ERROR_SUCCESS`: The GsTaichi C-API invocation finished gracefully.
- `TI_ERROR_NOT_SUPPORTED`: The invoked API, or the combination of parameters is not supported by the GsTaichi C-API.
...
- `TI_ERROR_INVALID_STATE`: The GsTaichi C-API enters an unrecoverable invalid state. Related GsTaichi objects are potentially corrupted. The users *should* release the contaminated resources for stability. Please feel free to file an issue if you encountered this error in a normal routine.
...
Usages of a memory allocation. GsTaichi requires kernel argument memories to be allocated with `TI_MEMORY_USAGE_STORAGE_BIT`.
...
Usages of an image allocation. GsTaichi requires kernel argument images to be allocated with `TI_IMAGE_USAGE_STORAGE_BIT` and `TI_IMAGE_USAGE_SAMPLED_BIT`.
...
> Stable since GsTaichi version: 1.5.0
...
Creates a GsTaichi Runtime with the specified [`TiArch`](#enumeration-tiarch).

- `arch`: Arch of GsTaichi Runtime.
- `device_index`: The index of device in `arch` to create GsTaichi Runtime on.
