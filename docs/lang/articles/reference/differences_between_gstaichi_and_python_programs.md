...
# Differences between GsTaichi and Python Programs

Although GsTaichi uses Python as the frontend, it follows a different set of rules in many aspects, including:

1. [GsTaichi only supports return statement outside non-static `if`/`for`/`while` scope in the program](#return-statement-and-return-type-annotation)
2. [Variables defined inside an `if`/`for`/`while` block cannot be accessed outside the block.](#variable-scoping)
3. [GsTaichi does not fully support some language features of Python.](#unsupportedpartially-supported-python-language-features)
...
- If a GsTaichi kernel/function does not have a return statement, it must not have return type annotation.
- If a GsTaichi kernel has a return statement, it must have return type annotation.
- If a GsTaichi function has a return statement, return type annotation is recommended, and it will be mandatory in the future.
...
**However**, in GsTaichi, the variables can only be accessed **within the block it is defined**.
...
Currently, GsTaichi does not support `set`.

List and dictionary before assigning to a variable works as the python list and dictionary.
However, after assigning to a variable, the content of the list and the values (not keys) of the dictionary are converted to GsTaichi variables.

GsTaichi does not have a runtime implementation of `in` currently. Therefore, operator `in` and `not in` only works in  [static scope](../advanced/meta.md#static-scope) (inside `ti.static()`).
...
GsTaichi partially supports list comprehension and dictionary comprehension,
...
Currently, GsTaichi does not support operator `is` and `is not`.
