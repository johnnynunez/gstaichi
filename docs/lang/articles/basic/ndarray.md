...
The GsTaichi ndarray is an array object that holds contiguous multi-dimensional data. Generally speaking, it plays a similar role to its counterpart `numpy.ndarray` in NumPy, but its underlying memory is allocated on the user-specified GsTaichi arch and managed by GsTaichi runtime.
...
The statement below instantiates an instance of a GsTaichi ndarray. `dtype` refers to the data type, which can either be a scalar data type like `ti.f32/ti.i32` or a vector/matrix data type like `ti.math.vec2/mat2`. `shape` denotes the array size with respect to the data type. Like fields, ndarray can only be constructed in the Python scope rather than in the GsTaichi scope. That is to say, an ndarray cannot be constructed inside GsTaichi kernels or functions.
...
Apart from the constructor, GsTaichi provides some basic operations to interact with ndarray data from the Python scope.
...
:::note
Accessing elements of an ndarray from the Python scope can be convenient, but it can also result in the creation and launch of multiple small GsTaichi kernels. This is not the most efficient approach from a performance standpoint. It is recommended that computationally intensive tasks be performed within a single GsTaichi kernel rather than operating on array elements individually from the Python scope.
:::
...
- Bidirectional data exchange with NumPy ndarrays
    ...
    # from_numpy copies the data in the NumPy array e to the GsTaichi ndarray d
    e.fill(10.0)  # Fills in the NumPy array with value 10.0
    d.from_numpy(e)  # Now d is filled in with 10.0
    ```

## Using GsTaichi ndarrays in `ti.kernel`

To use an ndarray in a GsTaichi kernel, you need to properly annotate its type in the kernel definition and pass the `Ndarray` object to the GsTaichi kernel at runtime. `Ndarray`s are passed by reference. Therefore, you can mutate their content inside GsTaichi kernels. The following example shows how to specify the type annotation for an ndarray:
...
The introduction of GsTaichi ndarrays achieves the seemless interop with external data containers. With the argument annotation `ti.types.ndarray`, a kernel accepts not only GsTaichi ndarrays but also external arrays. Currently, the external arrays supported by GsTaichi are NumPy ndarrays and PyTorch tensors.

The code snippet below defines a GsTaichi kernel that adds `1.0` to every element in the ndarray `arr`.
...
The external arrays can be fed into the GsTaichi kernel without further type conversions.

To feed a NumPy ndarray:

```python cont
arr_np = np.ones((3, 3), dtype=np.float32)
add_one(arr_np) # arr_np is updated by gstaichi kernel
```
...
add_one(arr_torch) # arr_torch is updated by gstaichi kernel
```

:::note

Every element in the external array (`arr_np` and `arr_torch`) is added by `1.0` when the GsTaichi kernel `add_one` finishes.

:::

When the external data container and GsTaichi are utilizing the same device, passing arguments incurs no additional overhead. This can be seen in the PyTorch example above, where the tensor is allocated on the CUDA device, which is also the device utilized by GsTaichi. As a result, the kernel function can access and manipulate the data in the original CUDA buffer allocated by PyTorch without incurring any extra costs.

On the other hand, if the devices being used are different, as is the case in the first example where Numpy utilizes CPUs and GsTaichi utilizes CUDA, GsTaichi automatically manages the transfer of data between devices, eliminating the need for manual intervention on the part of the user.
...
When a NumPy ndarray or a PyTorch tensor of scalar type is passed as the argument to a GsTaichi kernel, it can be interpreted as an array of scalar type, or an array of vector type, or an array of matrix type. This is controlled by the `dtype` and `ndim` options in the type hint `ti.types.ndarray()`.
...
Consider `ti.types.ndarray()` as a template type on parameters `dtype` and `ndim`. Equipped with a just-in-time (JIT) compiler, GsTaichi goes through the following two steps when a kernel with templated ndarray parameters is invoked:

1. First, GsTaichi checks whether a kernel with the same `dtype` and `ndim` inputs has been compiled. If yes, it loads and launches the compiled kernel directly with the input arguments.
...
We recommend referring to [this project](https://github.com/taichi-dev/gstaichi-nerfs/blob/main/notebooks/autodiff.ipynb).

Currently, the support for automatic differentiation in GsTaichi's ndarray is still incomplete. We are working on improving this functionality and will provide a more detailed tutorial as soon as possible.
