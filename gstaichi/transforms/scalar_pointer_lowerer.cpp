...
#include "gstaichi/transforms/utils.h"
...
void ScalarPointerLowerer::run() {
  std::array<int, gstaichi_max_num_indices> total_shape;
  total_shape.fill(1);
  for (const auto *s : snodes_) {
    for (int j = 0; j < gstaichi_max_num_indices; j++) {
      total_shape[j] *= s->extractors[j].shape;
    }
  }
  std::array<bool, gstaichi_max_num_indices> is_first_extraction;
