  ...
  for (int i = 0; i < paramter_list.size(); ++i) {
    ...
    if (symbolic_arg.tag == aot::ArgKind::kNdarray) {
      ...
      switch (type_size) {
        case 1:
          ctx.set_arg({i},
                      gstaichi_union_cast_with_different_sizes<int8>(ival.val));
          break;
        case 2:
          ctx.set_arg({i},
                      gstaichi_union_cast_with_different_sizes<int16>(ival.val));
          break;
        case 4:
          ctx.set_arg({i},
                      gstaichi_union_cast_with_different_sizes<int32>(ival.val));
          break;
        case 8:
          ctx.set_arg({i},
                      gstaichi_union_cast_with_different_sizes<int64>(ival.val));
          break;
        default:
