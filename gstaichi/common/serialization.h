...
#include <vector>
#include "gstaichi/common/json.h"
#include "gstaichi/common/json_serde.h"
#include "gstaichi/common/zip.h"
...
class BinarySerializer : public Serializer {
 ...
 private:
  ...
  // Unique Pointers to non-gstaichi-unit Types
  template <typename T>
  ...
  // Unique Pointers to gstaichi-unit Types
  template <typename T>
  typename std::enable_if<type::is_unit<T>::value, void>::type process(
      const std::unique_ptr<T> &val_) {
    auto &val = get_writable(val_);
