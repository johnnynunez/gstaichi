...
Expr cast(const Expr &input) {
  return gstaichi::lang::cast(input, get_data_type<T>());
}
...
Expr bit_cast(const Expr &input) {
  return gstaichi::lang::bit_cast(input, get_data_type<T>());
}
...
template <typename T>
Expr expr_rand() {
  return gstaichi::lang::expr_rand(get_data_type<T>());
}
