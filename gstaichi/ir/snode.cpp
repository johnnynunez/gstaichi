...
SNode &SNode::create_node(std::vector<Axis> axes,
  ...
  if (type == SNodeType::hash && depth != 0) {
    ErrorEmitter(GsTaichiRuntimeError(), &dbg_info,
  ...
  for (int i = 0; i < (int)axes.size(); i++) {
    if (sizes[i] <= 0) {
      ErrorEmitter(GsTaichiRuntimeError(), &dbg_info,
                   fmt::format("Every dimension of a GsTaichi field should be "
    ...
    if (is_first_division) {
      ...
      ErrorEmitter(
          GsTaichiRuntimeWarning(), &dbg_info,
          fmt::format(
  ...
  // infer extractors
  int64 acc_shape = 1;
  for (int i = gstaichi_max_num_indices - 1; i >= 0; i--) {
