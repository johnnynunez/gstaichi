...
class Axis {
 public:
  ...
  explicit Axis(int value) : value(value) {
    TI_ERROR_UNLESS(0 <= value && value < gstaichi_max_num_indices,
                    "Too many dimensions. The maximum dimensionality is {}",
                    gstaichi_max_num_indices);
  }
};
...
class SNode {
 public:
  ...
  AxisExtractor extractors[gstaichi_max_num_indices];
  std::vector<int> index_offsets;
  int num_active_indices{0};
  int physical_index_position[gstaichi_max_num_indices]{};
