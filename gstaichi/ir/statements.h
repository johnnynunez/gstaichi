 ...
 * When GsTaichi runtime initializes, each CUDA thread / CPU thread gets a
...
// TODO: remove this
// (penguinliong) This Stmt is used for both ND-arrays. This is
...
/**
 * A hint to the GsTaichi compiler about the relation of the values of two
 * statements.
 * This statement simply returns the input statement at the backend, and hints
 * the GsTaichi compiler that |base| + |low| <= |input| < |base| + |high|.
 */
...
/**
 * A hint to the GsTaichi compiler that a statement has unique values among
 * the top-level loop. This statement simply returns the input statement at
 * the backend, and hints the GsTaichi compiler that this statement never
...
/**
 * Call an inline GsTaichi function.
 */
