...
#include "gstaichi/python/snode_registry.h"
#include "gstaichi/program/sparse_matrix.h"
#include "gstaichi/program/sparse_solver.h"
#include "gstaichi/program/conjugate_gradient.h"
      ...
      .def("get_physical_index_position",
           [](SNode *snode) {
             return std::vector<int>(
                 ...
                 snode->physical_index_position + gstaichi_max_num_indices);
           })
...
#define PER_C_TYPE(type, ctype)                                           \
  case PrimitiveTypeID::type:                                             \
    ...
    break;
#include "gstaichi/inc/data_type_with_c_type.inc.h"
#undef PER_C_TYPE
            default:
...
#define PER_TYPE(x)                                                  \
  m.attr(("DataType_" + data_type_name(PrimitiveType::x)).c_str()) = \
      PrimitiveType::x;
#include "gstaichi/inc/data_type.inc.h"
#undef PER_TYPE
  ...
  m.def("get_max_num_indices", [] { return gstaichi_max_num_indices; });
  m.def("get_max_num_args", [] { return gstaichi_max_num_args; });
...
#define PER_INTERNAL_OP(x)                                           \
  internalOpClass.def_property_readonly_static(                      \
      ...
      py::return_value_policy::reference);
#include "gstaichi/inc/internal_ops.inc.h"
