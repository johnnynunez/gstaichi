...
#define RHI_VERIFY(rhi_call)                                    \
  {                                                             \
    gstaichi::lang::RhiResult r = rhi_call;                       \
    TI_ASSERT_INFO(r == gstaichi::lang::RhiResult::success,       \
...
inline GLFWwindow *create_glfw_window_(const std::string &name,
                                       ...
                                       int window_pos_y,
                                       bool vsync) {
  if (!gstaichi::lang::window_system::glfw_context_acquire()) {
  ...
  if (!window) {
    gstaichi::lang::window_system::glfw_context_release();
    exit(EXIT_FAILURE);
  }
