...
void Window::copy_depth_buffer_to_ndarray(
    const gstaichi::lang::Ndarray &depth_arr) {
  if (!drawn_frame_) {
    draw_frame();
  }

  if (depth_arr.dtype != gstaichi::lang::PrimitiveType::f32) {
  ...
  if (depth_arr.ndarray_alloc_ == gstaichi::lang::kDeviceNullAllocation) {
