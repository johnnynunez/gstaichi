...
Gui::Gui(AppContext *app_context, SwapChain *swap_chain, GsTaichiWindow *window) {
...
void Gui::init_render_resources(VkRenderPass render_pass) {
  ...
  auto &device =
      static_cast<gstaichi::lang::vulkan::VulkanDevice &>(app_context_->device());
...
void Gui::draw(gstaichi::lang::CommandList *cmd_list) {
  // Rendering
  ImGui::Render();
