...
void SetImage::update_data(const SetImageInfo &info) {
  ...
  TI_ASSERT_INFO(
      img.shape.size() == 2 && img.dtype == gstaichi::lang::PrimitiveType::u32,
  ...
  auto copy_op = [&, img_dev_ptr, uses_host](Device *device,
    ...
    // these are flipped because gstaichi is y-major and vulkan is x-major
...
void SetImage::update_data(Texture *tex) {
  ...
  // Reminder: y/x is flipped in GsTaichi. I would like to use the correct
