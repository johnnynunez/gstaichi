...
class TI_DLL_EXPORT GUI : public GUIBase {
 public:
  ...
  void update() {
    frame_id++;
    if (show_gui) {
      gstaichi::Time::wait_until(last_frame_time + frame_delta_limit);
      auto this_frame_time = gstaichi::Time::get_time();
