...
#include "gstaichi/platform/mac/objc_api.h"
...
#include "gstaichi/ui/gui/cocoa_keycodes.h"

namespace {
using gstaichi::mac::call;
using gstaichi::mac::cast_call;
using gstaichi::mac::clscall;
...
constexpr char kGsTaichiViewClassName[] = "GsTaichiGuiView";
...
class IdComparator {
 public:
  bool operator()(id a, id b) const {
    TI_STATIC_ASSERT(sizeof(a) == sizeof(gstaichi::int64));
    return gstaichi::bit::reinterpret_bits<gstaichi::int64>(a) <
           gstaichi::bit::reinterpret_bits<gstaichi::int64>(b);
  }
};

std::map<id, gstaichi::GUI *, IdComparator> gui_from_id;
...
void updateLayer(id self, SEL _) {
  ...
  // * profiling:
  // https://github.com/taichi-dev/gstaichi/issues/489#issuecomment-589955458
...
void GUI::create_window() {
  ...
  call(window, "initWithContentRect:styleMask:backing:defer:", rect,
       ...
       0, false);
  view = call(clscall(kGsTaichiViewClassName, "alloc"), "initWithFrame:", rect);
