...
void SparseMatrixBuilder::print_triplets_cuda() {
  ...
  for (auto i = 0; i < num_triplets_; i++) {
    int row = gstaichi_union_cast<int>(trips[3 * i + 1]);
    int col = gstaichi_union_cast<int>(trips[3 * i + 2]);
...
void EigenSparseMatrix<EigenMatrix>::build_triplets(void *triplets_adr) {
  std::string sdtype = gstaichi::lang::data_type_name(dtype_);
  if (sdtype == "f32") {
    BUILD(32)
  } else if (sdtype == "f64") {
    BUILD(64)
...
std::unique_ptr<SparseMatrix> make_sparse_matrix(
  ...
  std::string tdt = gstaichi::lang::data_type_name(dt);
...
void make_sparse_matrix_from_ndarray(Program *prog,
  ...
  std::string sdtype = gstaichi::lang::data_type_name(sm.get_data_type());
