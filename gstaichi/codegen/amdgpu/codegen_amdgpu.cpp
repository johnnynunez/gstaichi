...
#define UNARY_STD(x)                                                    \
    ...
    } else if (input_gstaichi_type->is_primitive(PrimitiveTypeID::f32)) { \
...
#undef UNARY_STD
  ...
  void visit(BinaryOpStmt *stmt) override {
    auto op = stmt->op_type;
    auto ret_gstaichi_type = stmt->ret_type;
    ...
    if (op == BinaryOpType::pow) {
      ...
      } else if (ret_gstaichi_type->is_primitive(PrimitiveTypeID::f32)) {
      ...
      } else if (ret_gstaichi_type->is_primitive(PrimitiveTypeID::i32)) {
        auto sitofp_lhs_ =
...
LLVMCompiledTask KernelCodeGenAMDGPU::compile_task(
    ...
    IRNode *block) {
  TaskCodeGenAMDGPU gen(task_codegen_id, config, get_gstaichi_llvm_context(),
                        kernel, block);
