...
#define UNARY_STD(x)                                                      \
    ...
    if (op == UnaryOpType::abs) {
      ...
      auto double_input =
          input_gstaichi_type->is_primitive(PrimitiveTypeID::f32)
              ? builder->CreateFPExt(
                    input,
      ...
      if (input_gstaichi_type->is_primitive(PrimitiveTypeID::u64) ||
          input_gstaichi_type->is_primitive(PrimitiveTypeID::i64)) {
        stmt->ret_type = PrimitiveType::i32;
        llvm_val[stmt] = call("__nv_popcll", input);
      } else if (input_gstaichi_type->is_primitive(PrimitiveTypeID::i32) ||
                 input_gstaichi_type->is_primitive(PrimitiveTypeID::u32)) {
        llvm_val[stmt] = call("__nv_popc", input);
      } else {
        TI_NOT_IMPLEMENTED
      }
    } else if (op == UnaryOpType::clz) {
      if (input_gstaichi_type->is_primitive(PrimitiveTypeID::i32)) {
        stmt->ret_type = PrimitiveType::i32;
  ...
  void visit(AtomicOpStmt *atomic_stmt) override {
    ...
    if (is_half2(dest_type) && is_half2(val_type) &&
      ...
      */
      ...
      // Defined in gstaichi/runtime/llvm/runtime_module/cuda_runtime.cu
...
LLVMCompiledTask KernelCodeGenCUDA::compile_task(
    ...
    IRNode *block) {
  TaskCodeGenCUDA gen(task_codegen_id, config, get_gstaichi_llvm_context(),
                      kernel, block);
