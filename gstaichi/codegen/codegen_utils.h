...
inline std::array<std::string, 5> parse_printf_specifier(std::string spec) {
  ...
  // Note that in gstaichi we support omitting the conversion, and the leading
  // '%' is ignored.
...
// Merges format specifiers with respect to user specified ones and gstaichi
// inferred ones.
...
//     e. nullopt_t (default),
//   as the specifier.
//   2. gstaichi will infer the specifier as '%.12f'.
