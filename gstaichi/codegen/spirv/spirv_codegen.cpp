...
void TaskCodegen::visit(ConstStmt *const_stmt) {
  auto get_const = [&](const TypedConstant &const_val) {
    ...
    if (dt->is_primitive(PrimitiveTypeID::f32)) {
      ...
      // Ref: See gstaichi::lang::TypedConstant::TypedConstant()
        ...
        if (auto struct_type = type->cast<gstaichi::lang::StructType>()) {
          for (int i = 0; i < struct_type->elements().size(); ++i) {
            ...
            indices.pop_back();
                       ...
                       type->cast<gstaichi::lang::TensorType>()) {
...
void TaskCodegen::visit(AtomicOpStmt *stmt) {
  ...
  if (dt->is_primitive(PrimitiveTypeID::f64)) {
    if (caps_->get(DeviceCapability::spirv_has_atomic_float64_add) &&
      ...
      addr_ptr = dest_is_ptr
                     ? dest_val
                     : at_buffer(stmt->dest, ir_->get_gstaichi_uint_type(dt));
    }
  ...
  if (is_real(dt)) {
    ...
    if (stmt->op_type == AtomicOpType::add) {
      ...
      addr_ptr = at_buffer(stmt->dest, ir_->get_gstaichi_uint_type(dt));
...
void TaskCodegen::compile_args_struct() {
  ...
  std::unordered_map<std::vector<int>, const tinyir::Type *,
                     hashing::Hasher<std::vector<int>>>
      element_types;
  std::unordered_map<std::vector<int>, const gstaichi::lang::Type *,
                     hashing::Hasher<std::vector<int>>>
      element_gstaichi_types;
  ...
  std::function<void(const std::vector<int> &indices, const Type *type)>
      add_types_to_element_types =
          [&](const std::vector<int> &indices, const Type *type) {
            ...
            element_gstaichi_types[indices] = type;
  ...
  for (auto &element : element_types) {
    ...
    spirv_type.dt = element_gstaichi_types[element.first];
