...
struct SType {
  ...
  // corresponding GsTaichi type/Compiled SNode info
  DataType dt;
...
class IRBuilder {
 public:
  ...
  // Get null stype
  SType get_null_type();
  // Get the spirv type for a given GsTaichi primitive data type
  SType get_primitive_type(const DataType &dt) const;
  // Get the spirv type for a given GsTaichi data type
  SType from_gstaichi_type(const DataType &dt, bool has_buffer_ptr);
  // Get the size in bytes of a given GsTaichi data type
  size_t get_primitive_type_size(const DataType &dt) const;
  // Get the spirv uint type with the same size of a given GsTaichi data type
  SType get_primitive_uint_type(const DataType &dt) const;
  // Get the GsTaichi uint type with the same size of a given GsTaichi data type
  DataType get_gstaichi_uint_type(const DataType &dt) const;
