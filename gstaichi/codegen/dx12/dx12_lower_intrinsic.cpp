...
#define DEBUG_TYPE "dxil-gstaichi-intrinsic-lower"

namespace {

class GsTaichiIntrinsicLower : public ModulePass {
 public:
  bool runOnModule(Module &M) override {
    ...
    auto patch_intrinsic = [&](std::string name, Intrinsic::ID intrin,
      ...
      gstaichi::lang::GsTaichiLLVMContext::mark_inline(func);
    };
    ...
    // GsTaichiRuntimeContextLower pass will place the CBuffer to special binding
    // space.
  ...
  GsTaichiIntrinsicLower(const gstaichi::lang::CompileConfig *config = nullptr)
      : ModulePass(ID), config(config) {
    initializeGsTaichiIntrinsicLowerPass(*PassRegistry::getPassRegistry());
  }
...
char GsTaichiIntrinsicLower::ID = 0;

}  // end anonymous namespace

INITIALIZE_PASS(GsTaichiIntrinsicLower,
                DEBUG_TYPE,
                "Lower gstaichi intrinsic",
                false,
                false)

llvm::ModulePass *llvm::createGsTaichiIntrinsicLowerPass(
    const gstaichi::lang::CompileConfig *config) {
  return new GsTaichiIntrinsicLower(config);
}
