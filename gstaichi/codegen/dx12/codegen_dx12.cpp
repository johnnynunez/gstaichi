...
#include "gstaichi/codegen/dx12/codegen_dx12.h"
#include "gstaichi/codegen/dx12/dx12_llvm_passes.h"
...
static std::vector<uint8_t> generate_dxil_from_llvm(
    ...
    const CompileConfig &config,
    const gstaichi::lang::Kernel *kernel) {
...
LLVMCompiledTask KernelCodeGenDX12::compile_task(
    ...
    IRNode *block) {
  TaskCodeGenLLVMDX12 gen(task_codegen_id, config, get_gstaichi_llvm_context(),
                          kernel, block);
