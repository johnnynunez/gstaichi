...
void TaskCodeGenLLVM::emit_extra_unary(UnaryOpStmt *stmt) {
  ...
  auto input_gstaichi_type = stmt->operand->ret_type;
  if (input_gstaichi_type->is_primitive(PrimitiveTypeID::f16)) {
    ...
    input_gstaichi_type = PrimitiveType::f32;
  }
...
void TaskCodeGenLLVM::visit(LocalLoadStmt *stmt) {
  // FIXME: get ptr_ty from gstaichi instead of llvm.
...
void TaskCodeGenLLVM::visit(AssertStmt *stmt) {
  TI_ASSERT((int)stmt->args.size() <= gstaichi_error_message_max_num_arguments);
  ...
  llvm_val[stmt] = call("gstaichi_assert_format", std::move(args));
}
...
void TaskCodeGenLLVM::create_offload_struct_for(OffloadedStmt *stmt) {
...

  {
     ...
     *
     * struct_for_body:
     *   ... (Run codegen on the StructForStmt::body GsTaichi Block)
     *   goto loop_body_tail
     *
     * loop_body_tail:
  ...
  int list_element_size = std::min(leaf_block->max_num_elements(),
                                   (int64)gstaichi_listgen_max_element_size);
...
void TaskCodeGenLLVM::visit(LoopIndexStmt *stmt) {
  if (stmt->loop->is<OffloadedStmt>() &&
    ...
    // FIXME: get struct_ty from gstaichi instead of llvm.
...
void TaskCodeGenLLVM::eliminate_unused_functions() {
  GsTaichiLLVMContext::eliminate_unused_functions(
...
LLVMCompiledTask TaskCodeGenLLVM::run_compilation() {
  ...
  if (get_environ_config(LOAD_IR_ENV.data())) {
    ...
    if (!loaded_module) {
      err.print("GSTAICHI_LOAD_IR_FILE error", llvm::errs());
