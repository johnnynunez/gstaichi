...
#include "gstaichi/common/dynamic_loader.h"
...
#else
  static_assert(false, "GsTaichi CUDA driver supports only Windows and Linux.");
#endif
...
#include "gstaichi/rhi/cuda/cuda_driver_functions.inc.h"
...
#define PER_CUSPARSE_FUNCTION(name, symbol_name, ...) \
  ...
  name.set_lock(&lock_);                              \
  name.set_names(#name, #symbol_name);
#include "gstaichi/rhi/cuda/cusparse_functions.inc.h"
...
#define PER_CUSOLVER_FUNCTION(name, symbol_name, ...) \
  ...
  name.set_lock(&lock_);                              \
  name.set_names(#name, #symbol_name);
#include "gstaichi/rhi/cuda/cusolver_functions.inc.h"
...
#define PER_CUBLAS_FUNCTION(name, symbol_name, ...) \
  ...
  name.set_lock(&lock_);                            \
  name.set_names(#name, #symbol_name);
#include "gstaichi/rhi/cuda/cublas_functions.inc.h"
