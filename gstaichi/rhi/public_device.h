...
class RHI_DLL_EXPORT GraphicsDevice : public Device {
 public:
  ...
  // in a gstaichi kernel, you usually want to create the image via
...
template <>
class fmt::formatter<gstaichi::lang::RhiResult> {
 public:
  constexpr auto parse(format_parse_context &ctx) {
    return ctx.begin();
  }
  template <typename Context>
  constexpr auto format(gstaichi::lang::RhiResult const &res,
                        Context &ctx) const {
    return format_to(ctx.out(), gstaichi::lang::rhi_result_to_string(res));
  }
};
