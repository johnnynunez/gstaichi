...
// Info for launching a compiled GsTaichi kernel, which consists of a series of
// Unified Device API pipelines.

CompiledGsTaichiKernel::CompiledGsTaichiKernel(const Params &ti_params)
...
size_t CompiledGsTaichiKernel::num_pipelines() const {
  return pipelines_.size();
}

size_t CompiledGsTaichiKernel::get_args_buffer_size() const {
  return args_buffer_size_;
}

size_t CompiledGsTaichiKernel::get_ret_buffer_size() const {
  return ret_buffer_size_;
}

Pipeline *CompiledGsTaichiKernel::get_pipeline(int i) {
...
GfxRuntime::KernelHandle GfxRuntime::register_gstaichi_kernel(
    GfxRuntime::RegisterParams reg_params) {
  CompiledGsTaichiKernel::Params params;
  ...
  ti_kernels_.push_back(std::make_unique<CompiledGsTaichiKernel>(params));
  return res;
}
...
void GfxRuntime::launch_kernel(KernelHandle handle,
  ...
  if (profiler_) {
    ...
    if (task_count > apple_max_query_pool_count) {
      TI_WARN(
          "Cannot concurrently profile more than 32 tasks in a single GsTaichi "
