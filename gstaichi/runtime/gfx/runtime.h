...
using namespace gstaichi::lang::spirv;
...
class CompiledGsTaichiKernel {
 public:
  struct Params {
    const GsTaichiKernelAttributes *ti_kernel_attribs{nullptr};
  ...
  explicit CompiledGsTaichiKernel(const Params &ti_params);

  const GsTaichiKernelAttributes &ti_kernel_attribs() const;
 ...
 private:
  GsTaichiKernelAttributes ti_kernel_attribs_;
...
class TI_DLL_EXPORT GfxRuntime {
 public:
  ...
  struct RegisterParams {
    GsTaichiKernelAttributes kernel_attribs;
    std::vector<std::vector<uint32_t>> task_spirv_source_codes;
    std::size_t num_snode_trees{0};
  };

  KernelHandle register_gstaichi_kernel(RegisterParams params);
 ...
 private:
  friend class gstaichi::lang::gfx::SNodeTreeManager;
  ...
  std::vector<std::unique_ptr<CompiledGsTaichiKernel>> ti_kernels_;
  ...
  // Since we separate cmdlist is async, gstaichi needs a way to know whether
