...
std::string JITSessionAMDGPU::compile_module_to_hsaco(
  ...
  if (this->config_.print_kernel_llvm_ir) {
    static FileSequenceWriter writer("gstaichi_kernel_amdgpu_llvm_ir_{:04d}.ll",
  ...
  auto obj_filename = "gstaichi_amdgcn_" + std::to_string(random_num) + ".o";
  auto hsaco_filename =
      "gstaichi_amdgcn_" + std::to_string(random_num) + ".hsaco";
  ...
  if (this->config_.print_kernel_llvm_ir_optimized) {
    static FileSequenceWriter writer(
        "gstaichi_kernel_amdgpu_llvm_ir_optimized_{:04d}.ll",
        "unoptimized LLVM IR (AMDGPU)");
    writer.write(llvm_module.get());
  }
