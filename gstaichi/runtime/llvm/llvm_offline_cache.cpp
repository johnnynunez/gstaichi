...
static std::string get_llvm_cache_metadata_file_path(const std::string &dir) {
  return gstaichi::join_path(dir, std::string(kMetadataFilename) + ".tcb");
}

static std::string get_llvm_cache_metadata_json_file_path(
    const std::string &dir) {
  return gstaichi::join_path(dir, std::string(kMetadataFilename) + ".json");
}
...
bool LlvmOfflineCacheFileReader::load_meta_data(
  ...
  if (!gstaichi::path_exists(tcb_path)) {
  ...
  std::string lock_path =
      gstaichi::join_path(cache_file_path, kMetadataFileLockName);
...
bool LlvmOfflineCacheFileReader::get_kernel_cache(
  ...
  if (!data.module) {
    std::string filename_prefix = gstaichi::join_path(path_, key);
  ...
  if (!verified) {
    for (const auto &f : get_possible_llvm_cache_filename_by_key(key)) {
      gstaichi::remove(gstaichi::join_path(path_, f));
    }
  }
...
void LlvmOfflineCacheFileWriter::dump(const std::string &path,
  ...
  gstaichi::create_directories(path);
  ...
  for (auto iter = data_.kernels.begin(); iter != data_.kernels.end(); ++iter) {
    ...
    std::string filename_prefix = gstaichi::join_path(path, k);
    {
...

  {
    ...
    std::string lock_path = gstaichi::join_path(path, kMetadataFileLockName);
    if (!lock_with_file(lock_path)) {
      TI_WARN(
