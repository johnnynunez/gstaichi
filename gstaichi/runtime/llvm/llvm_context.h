...
/**
 * Manages an LLVMContext for GsTaichi's usage.
 */
class GsTaichiLLVMContext {
 ...
 public:
  ...
  GsTaichiLLVMContext(const CompileConfig &config, Arch arch);

  virtual ~GsTaichiLLVMContext();
  ...
  llvm::Type *get_data_type() {
    return GsTaichiLLVMContext::get_data_type(gstaichi::lang::get_data_type<T>());
  }
