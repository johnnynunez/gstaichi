...
// Note: ARM64 atomic operations are now handled by disabling outlined atomics
// via the -mno-outline-atomics compiler flag, eliminating the need for
// inline assembly workarounds. This provides better maintainability and
// leverages the compiler's optimized atomic implementations.
...
void gstaichi_printf(LLVMRuntime *runtime, const char *format, Args &&...args);
...
// implement it here.
std::size_t gstaichi_strlen(const char *str) {
  std::size_t len = 0;
  for (auto p = str; *p; p++)
    len++;
  return len;
...
struct PhysicalCoordinates {
  i32 val[gstaichi_max_num_indices];
};
...
#include "gstaichi/runtime/llvm/runtime_module/atomic.h"
...
void gstaichi_assert(RuntimeContext *context, u1 test, const char *msg);
void gstaichi_assert_runtime(LLVMRuntime *runtime, u1 test, const char *msg);
#define TI_ASSERT_INFO(x, msg) gstaichi_assert(context, (u1)(x), msg)
...
T debug_add(RuntimeContext *ctx, T a, T b, const char *tb) {
  ...
  if (__builtin_add_overflow(a, b, &c)) {
    gstaichi_printf(ctx->runtime, "Addition overflow detected in %s\n", tb);
  }
  return c;
}
...
T debug_sub(RuntimeContext *ctx, T a, T b, const char *tb) {
  ...
  if (__builtin_sub_overflow(a, b, &c)) {
    gstaichi_printf(ctx->runtime, "Subtraction overflow detected in %s\n", tb);
  }
  return c;
}
...
T debug_shl(RuntimeContext *ctx, T a, i32 b, const char *tb) {
  T c = a << b;
  if (c >> b != a) {
    gstaichi_printf(ctx->runtime, "Shift left overflow detected in %s\n", tb);
  }
  return c;
}
...
struct ListManager {
  ...
  i32 ptr2index(Ptr ptr) {
    ...
    for (int i = 0; i < max_num_chunks; i++) {
      gstaichi_assert_runtime(runtime, chunks[i] != nullptr, "ptr not found.");
...
struct LLVMRuntime {
  ...
  ListManager *element_lists[gstaichi_max_num_snodes];
  NodeManager *node_allocators[gstaichi_max_num_snodes];
  Ptr ambient_elements[gstaichi_max_num_snodes];
  Ptr temporaries;
  ...
  char error_message_template[gstaichi_error_message_max_length];
  uint64 error_message_arguments[gstaichi_error_message_max_num_arguments];
  ...
  void set_result(std::size_t i, T t) {
    ...
    ((u64 *)result_buffer)[i] =
        gstaichi_union_cast_with_different_sizes<uint64>(t);
  }
...
void RuntimeContext_store_result(RuntimeContext *ctx, u64 ret, u32 idx) {
  ctx->result_buffer[gstaichi_result_buffer_ret_value_id + idx] = ret;
}
...
void runtime_retrieve_and_reset_error_code(LLVMRuntime *runtime) {
  runtime->set_result(gstaichi_result_buffer_error_id, runtime->error_code);
  runtime->error_code = 0;
}

void runtime_retrieve_error_message(LLVMRuntime *runtime, int i) {
  runtime->set_result(gstaichi_result_buffer_error_id,
...
void runtime_ListManager_get_num_active_chunks(LLVMRuntime *runtime,
  ...
  runtime->set_result(gstaichi_result_buffer_runtime_query_id,
...
void gstaichi_assert(RuntimeContext *context, u1 test, const char *msg) {
  gstaichi_assert_runtime(context->runtime, test, msg);
}
...
void gstaichi_assert_format(LLVMRuntime *runtime,
    ...
    locked_task(&runtime->error_message_lock, [&] {
      if (!runtime->error_code) {
        ...
        memset(runtime->error_message_template, 0,
               gstaichi_error_message_max_length);
        memcpy(runtime->error_message_template, format,
               std::min(gstaichi_strlen(format),
                        gstaichi_error_message_max_length - 1));
  ...
  // (Aborted), which will be caught by GsTaichi's signal handler. The assert
  // failure message will NOT be properly printed since GsTaichi exits after
...
void gstaichi_assert_runtime(LLVMRuntime *runtime, u1 test, const char *msg) {
  gstaichi_assert_format(runtime, test, msg, 0, nullptr);
}
...
Ptr LLVMRuntime::allocate_from_reserved_memory(
  ...
  if (!success) {
    ...
    // halt the whole grid. Using a gstaichi_assert_runtime will not finish the
    ...
    __assertfail(
        ...
        " GPU memory",
        "GsTaichi JIT", 0, "allocate_from_reserved_memory", 1);
#endif
  }
  gstaichi_assert_runtime(this, success, "Out of pre-allocated memory");
  return ret;
}
...
void runtime_initialize(
  ...
  if (preallocated_size) {
    ...
    preallocated_buffer +=
        gstaichi::iroundup(sizeof(LLVMRuntime), gstaichi_page_size);
  } else {
    runtime =
  ...
  runtime->set_result(gstaichi_result_buffer_ret_value_id, runtime);
  ...
  runtime->temporaries = (Ptr)runtime->allocate_aligned(
      runtime->runtime_objects_chunk, gstaichi_global_tmp_buffer_size,
      gstaichi_page_size);
  ...
  runtime->rand_states = (RandState *)runtime->allocate_aligned(
      ...
      sizeof(RandState) * runtime->num_rand_states, gstaichi_page_size);
}
...
void element_listgen_root(LLVMRuntime *runtime,
  ...
  auto ch_element_size =
      std::min(ch_num_elements, gstaichi_listgen_max_element_size);
...
void cpu_parallel_range_for(RuntimeContext *context,
  ...
  ctx.step = step;
  if (step != 1 && step != -1) {
    gstaichi_printf(context->runtime, "step must not be %d\n", step);
    exit(-1);
  }
...
f64 rounding_prepare_f64(f64 f) {
  // Same as above
  i64 delta_bits = (gstaichi_union_cast<i64>(f) & 0x8000000000000000LL) |
                   gstaichi_union_cast<i64>(0.5);
  f64 delta = gstaichi_union_cast<f64>(delta_bits);
  return f + delta;
}
}
