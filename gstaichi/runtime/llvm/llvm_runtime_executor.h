...
#include "gstaichi/runtime/llvm/snode_tree_buffer_manager.h"
...
class LlvmRuntimeExecutor {
 public:
  ...
  GsTaichiLLVMContext *get_llvm_context();
 ...
 private:
  ...
  T fetch_result(int i, uint64 *result_buffer) {
    return gstaichi_union_cast_with_different_sizes<T>(
  ...
  T runtime_query(const std::string &key,
    ...
    return gstaichi_union_cast_with_different_sizes<T>(fetch_result_uint64(
        gstaichi_result_buffer_runtime_query_id, result_buffer));
  }
 ...
 private:
  ...
  std::unique_ptr<GsTaichiLLVMContext> llvm_context_{nullptr};
