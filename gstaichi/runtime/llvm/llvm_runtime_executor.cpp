...
#else
    TI_WARN("GsTaichi is not compiled with AMDGPU.");
    config.arch = host_arch();
#endif
  }
...
GsTaichiLLVMContext *LlvmRuntimeExecutor::get_llvm_context() {
...
void LlvmRuntimeExecutor::check_runtime_error(uint64 *result_buffer) {
  ...
  runtime_jit_module->call<void *>("runtime_retrieve_and_reset_error_code",
                                   llvm_runtime_);
  auto error_code =
      fetch_result<int64>(gstaichi_result_buffer_error_id, result_buffer);
      ...
      const auto error_message_formatted = format_error_message(
          ...
          [runtime_jit_module, result_buffer, this](int argument_id) {
            runtime_jit_module->call<void *>(
                ...
                argument_id);
            return fetch_result<uint64>(gstaichi_result_buffer_error_id,
                                        result_buffer);
          });
      throw GsTaichiAssertionError(error_message_formatted);
    } else {
      TI_NOT_IMPLEMENTED
    }
  }
...
void LlvmRuntimeExecutor::initialize_llvm_runtime_snodes(
  ...
  std::size_t rounded_size = gstaichi::iroundup(root_size, gstaichi_page_size);
...
void LlvmRuntimeExecutor::materialize_runtime(KernelProfilerBase *profiler,
  ...
  llvm_runtime_ = fetch_result<void *>(gstaichi_result_buffer_ret_value_id,
                                       *result_buffer_ptr);
