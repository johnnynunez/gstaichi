...
GsTaichiLLVMContext::GsTaichiLLVMContext(const CompileConfig &config, Arch arch)
    : config_(config), arch_(arch) {
  TI_TRACE("Creating GsTaichi llvm context for arch: {}", arch_name(arch));
  ...
  data_layout_ = GsTaichiLLVMContext::get_data_layout(arch);
  ...
  TI_TRACE("GsTaichi llvm context created.");
}

GsTaichiLLVMContext::~GsTaichiLLVMContext() {
}

llvm::Type *GsTaichiLLVMContext::get_data_type(DataType dt) {
...
GsTaichiLLVMContext::clone_module_to_this_thread_context(llvm::Module *module) {
...
std::unique_ptr<llvm::Module> LlvmModuleBitcodeLoader::load(
  ...
  if (inline_funcs_) {
    for (auto &f : *(runtime.get())) {
      GsTaichiLLVMContext::mark_inline(&f);
    }
  }
...
std::unique_ptr<llvm::Module> GsTaichiLLVMContext::clone_runtime_module() {
  TI_AUTO_PROF
...
std::unique_ptr<llvm::Module> GsTaichiLLVMContext::module_from_file(
...
void GsTaichiLLVMContext::link_module_with_custom_cuda_library(
...
void GsTaichiLLVMContext::link_module_with_cuda_libdevice(
    std::unique_ptr<llvm::Module> &module) {
  TI_AUTO_PROF
...
void GsTaichiLLVMContext::link_module_with_amdgpu_libdevice(
    std::unique_ptr<llvm::Module> &module) {
...
template llvm::Value *GsTaichiLLVMContext::get_constant(DataType dt, int32 t);
template llvm::Value *GsTaichiLLVMContext::get_constant(DataType dt, int64 t);
template llvm::Value *GsTaichiLLVMContext::get_constant(DataType dt, uint32 t);
template llvm::Value *GsTaichiLLVMContext::get_constant(DataType dt, uint64 t);
template llvm::Value *GsTaichiLLVMContext::get_constant(DataType dt, float32 t);
template llvm::Value *GsTaichiLLVMContext::get_constant(DataType dt, float64 t);
...
std::string GsTaichiLLVMContext::type_name(llvm::Type *type) {
...
std::size_t GsTaichiLLVMContext::get_type_size(llvm::Type *type) {
...
void GsTaichiLLVMContext::mark_inline(llvm::Function *f) {
  for (auto &B : *f)
    for (auto &I : B) {
...
int GsTaichiLLVMContext::num_instructions(llvm::Function *func) {
  int counter = 0;
...
void GsTaichiLLVMContext::print_huge_functions(llvm::Module *module) {
  int total_inst = 0;
...
llvm::DataLayout GsTaichiLLVMContext::get_data_layout() {
  return data_layout_;
}
...
void GsTaichiLLVMContext::mark_function_as_amdgpu_kernel(llvm::Function *func) {
  func->setCallingConv(llvm::CallingConv::AMDGPU_KERNEL);
}

void GsTaichiLLVMContext::eliminate_unused_functions(
    llvm::Module *module,
...
llvm::LLVMContext *GsTaichiLLVMContext::get_this_thread_context() {
...
GsTaichiLLVMContext::get_this_thread_thread_safe_context() {
  get_this_thread_context();  // make sure the context is created
  ThreadLocalData *data = get_this_thread_data();
  return data->thread_safe_llvm_context.get();
}

template llvm::Value *GsTaichiLLVMContext::get_constant(float32 t);
template llvm::Value *GsTaichiLLVMContext::get_constant(float64 t);

template llvm::Value *GsTaichiLLVMContext::get_constant(bool t);

template llvm::Value *GsTaichiLLVMContext::get_constant(int32 t);
template llvm::Value *GsTaichiLLVMContext::get_constant(uint32 t);

template llvm::Value *GsTaichiLLVMContext::get_constant(int64 t);
template llvm::Value *GsTaichiLLVMContext::get_constant(uint64 t);
...
void GsTaichiLLVMContext::delete_snode_tree(int id) {
...
void GsTaichiLLVMContext::fetch_this_thread_struct_module() {
  ThreadLocalData *data = get_this_thread_data();
...
llvm::Function *GsTaichiLLVMContext::get_runtime_function(
    const std::string &name) {
  return get_this_thread_runtime_module()->getFunction(name);
}

llvm::Module *GsTaichiLLVMContext::get_this_thread_runtime_module() {
  TI_AUTO_PROF;
...
llvm::Type *GsTaichiLLVMContext::get_runtime_type(const std::string &name) {
...
std::unique_ptr<llvm::Module> GsTaichiLLVMContext::new_module(
    std::string name,
...
GsTaichiLLVMContext::ThreadLocalData::ThreadLocalData(
...
LLVMCompiledKernel GsTaichiLLVMContext::link_compiled_tasks(
...
std::string GsTaichiLLVMContext::get_struct_for_func_name(int tls_size) {
  return "parallel_struct_for_" + std::to_string(tls_size);
}

llvm::DataLayout GsTaichiLLVMContext::get_data_layout(Arch arch) {
...
std::string GsTaichiLLVMContext::get_data_layout_string() {
